...
public class myTaskStatic extends Fragment implements View.OnClickListener{
    ...
    public void  CheckEmpty(String getStartTime,String getEndTime,String getBreakStartTime,String getPeriodDuration,String getBreakDuration,String getAlarmbefore,int intStartTime,int intEndTime,int intBreakStartTime){
        ...
        else if((intBreakStartTime-intStartTime)%Integer.parseInt(getPeriodDuration)!=0){
            snackbar1 = Snackbar.make(getView(), "Error! Check break start time it's not divisible by - "+getPeriodDuration+" !", Snackbar.LENGTH_SHORT);snackbar1.show();
        }
        else if((intEndTime-(intBreakStartTime+Integer.parseInt(getBreakDuration)))%Integer.parseInt(getPeriodDuration)!=0){
            snackbar1 = Snackbar.make(getView(), "Error!Check end time it's not divisible by - "+getPeriodDuration+" !", Snackbar.LENGTH_SHORT);snackbar1.show();
        }
        else {
